...
### Added
- **DiffusionRouter** (`src/models/diffusionrouter/`, `src/pipelines/diffusionrouter.py`, `examples/diffusionrouter/`, `examples/community/diffusionrouter/`): Universal multi-domain translation via conditional diffusion routers from [kvmduc/DiffusionRouter](https://github.com/kvmduc/DiffusionRouter) — routing config, multi-hop inference pipeline, and checkpoint loader (requires local `guided_diffusion` checkout).
