...
"""pix2pix-turbo training harness."""
...
from src.models.img2img_turbo.pix2pix_turbo import Pix2PixTurbo
from src.models.img2img_turbo.data import PairedTurboDataset
...
def main(args):
    ...
    if args.pretrained_model_name_or_path == "stabilityai/sd-turbo":
        net_pix2pix = Pix2PixTurbo(lora_rank_unet=args.lora_rank_unet, lora_rank_vae=args.lora_rank_vae)
    ...
    dataset_train = PairedTurboDataset(dataset_folder=args.dataset_folder, image_prep=args.train_image_prep, split="train", tokenizer=net_pix2pix.tokenizer)
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True, num_workers=args.dataloader_num_workers)
    dataset_val = PairedTurboDataset(dataset_folder=args.dataset_folder, image_prep=args.test_image_prep, split="test", tokenizer=net_pix2pix.tokenizer)
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Train pix2pix-turbo")
    ...
    parser.add_argument("--train_image_prep", default="resized_crop_512")
    parser.add_argument("--test_image_prep", default="resized_crop_512")
...
class Pix2PixTurboTrainer:
    """Training harness wrapping the upstream pix2pix-turbo loop."""

    def __init__(self, config: Pix2PixTurboConfig) -> None:
        self.config = config

    def train(self) -> None:
        ns = argparse.Namespace(**asdict(self.config))
        ns.pretrained_model_name_or_path = self.config.base_model
        main(ns)


if __name__ == "__main__":
    cli = _parse_args()
    base = Pix2PixTurboConfig()
    for k, v in vars(cli).items():
        if hasattr(base, k):
            setattr(base, k, v)
        elif k == "pretrained_model_name_or_path":
            base.base_model = v
    Pix2PixTurboTrainer(base).train()
