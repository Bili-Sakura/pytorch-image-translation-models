...
"""LBM trainer for flow-matching bridge (predict x_source - x_target)."""
...
class LBMTrainer:
    """LBM: model predicts flow (x_source - x_target)."""
    ...
    def __init__(self, config: LBMConfig) -> None:
        ...
        self.scheduler = LBMScheduler(
            num_train_timesteps=config.num_train_timesteps,
            bridge_noise_sigma=config.bridge_noise_sigma,
        )
    ...
    def train_step(self, source: torch.Tensor, target: torch.Tensor) -> dict:
        """LBM: add_noise(target, source, t), predict flow, MSE(flow, source-target)."""
        source = source.to(self.device) * 2 - 1
        target = target.to(self.device) * 2 - 1

        b = target.shape[0]
        timesteps = self.scheduler.sample_timesteps(b, device=self.device)
        x_t = self.scheduler.add_noise(target, source, timesteps)
        target_flow = self.scheduler.compute_target(source, target)

        pred_flow = self.unet(x_t, timesteps, cond=source)
    ...
    def train(
        ...
        logger.info("LBM training complete. Checkpoints saved to %s", cfg.save_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./checkpoints/lbm")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch-size", type=int, default=16)
    args = parser.parse_args()
    config = LBMConfig(save_dir=args.save_dir, epochs=args.epochs, batch_size=args.batch_size)
    trainer = LBMTrainer(config)
