...
"""BiBBDM trainer for bidirectional paired image-to-image translation."""
...
from .config import BiBBDMConfig
...
class BiBBDMTrainer:
    """Training harness for BiBBDM (Bidirectional Brownian Bridge)."""

    def __init__(self, config: BiBBDMConfig) -> None:
        self.config = config
        self.device = torch.device(config.device if torch.cuda.is_available() else "cpu")

        self.unet = BiBBDMUNet(
        ...
        self.scheduler = BiBBDMScheduler(
    ...
    def train(
        ...
        logger.info("BiBBDM training complete. Checkpoints saved to %s", cfg.save_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./checkpoints/bibbdm")
    parser.add_argument("--epochs", type=int, default=200)
    parser.add_argument("--batch-size", type=int, default=16)
    args = parser.parse_args()
    config = BiBBDMConfig(save_dir=args.save_dir, epochs=args.epochs, batch_size=args.batch_size)
    trainer = BiBBDMTrainer(config)
