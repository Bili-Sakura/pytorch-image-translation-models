...
This is a **community pipeline**: a self-contained, single-file module that bundles
all network definitions, losses, and a training helper so it can be used without
importing any other project code from ``src/``.

Paper
-----
    Qin J., Zou B., Li H., Zhang L.
    *Efficient End-to-End Diffusion Model for One-step SAR-to-Optical Translation*
    ...
    https://ieeexplore.ieee.org/document/10767752

Architecture
------------
E3Diff is a two-stage training approach:

* **Stage 1 – Diffusion pre-training** (``GaussianDiffusion``):
  A conditional UNet learns to denoise SAR-conditioned optical images using a standard
  DDPM loss (L1 on predicted noise) plus an optional Focal-Frequency loss. The conditioning
  signal is extracted by the Conditional Prior Enhancement Network (CPEN), a multi-scale
  encoder that produces five feature maps aligned with UNet decoder levels.

* **Stage 2 – One-step adversarial refinement** (``E3DiffTrainer`` with ``stage=2``):
  The Stage-1 model is fine-tuned for one-step DDIM inference using a GAN objective
  with a PatchGAN discriminator. This yields a single-forward-pass translation model.

Key modules:

* ``CPEN`` – Conditional Prior Enhancement Network; multi-scale encoder for the SAR
  conditioning image. Uses ``AvgPool2d`` (replacing the original ``SoftPool`` to remain
  self-contained). Produces feature maps at resolutions H, H/2, H/4, H/8, H/16.
* ``E3DiffUNet`` – The conditional denoising U-Net. Each residual block receives both a
  timestep embedding and a spatial feature from CPEN at the matching scale.
* ``GaussianDiffusion`` – Wraps the U-Net with the forward/reverse diffusion process and
  DDIM sampling. Supports Stage-1 (noise prediction) and Stage-2 (one-step DDIM).
* ``FocalFrequencyLoss`` – Frequency-domain loss that dynamically weights the spectrum
  of hard-to-reconstruct frequencies (Jiang et al., ICCV 2021).
* ``E3DiffTrainer`` – Convenience trainer encapsulating both stages.
...
.. code-block:: python
    ...
    cfg = E3DiffConfig(stage=1, device="cpu")
    trainer = E3DiffTrainer(cfg)
    losses = trainer.train_step(sar_image, optical_image)

    # Stage 2: fine-tune with GAN for one-step inference
    cfg2 = E3DiffConfig(stage=2, device="cpu")
    trainer2 = E3DiffTrainer(cfg2)
    losses2 = trainer2.train_step(sar_image, optical_image)

    # Inference (one-step DDIM)
    with torch.no_grad():
        prediction = trainer2.sample(sar_image)
