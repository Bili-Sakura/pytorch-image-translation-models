...
"""Convert raw LDDBM .pt checkpoints to project format (config.json + safetensors per subfolder).

Usage:
    python -m examples.lddbm.convert_pt_to_lddbm \\
        /path/to/lddbm-checkpoints \\
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert LDDBM .pt checkpoints to config.json + safetensors"
    )
    parser.add_argument(
        "output_dir",
        type=str,
        help="Output directory (e.g. lddbm-checkpoints/ or path/to/lddbm-safetensors)",
    )
