...
# Credits: img2img-turbo from Parmar et al. 2024 — https://github.com/GaParmar/img2img-turbo

"""CycleGAN-Turbo training harness."""
...
from src.models.img2img_turbo.utils import make_1step_sched
from src.models.img2img_turbo.cyclegan_turbo import CycleGANTurbo, VAEEncode, VAEDecode, initialize_unet, initialize_vae
from src.models.img2img_turbo.data import UnpairedTurboDataset, build_transform
...
def main(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        ...
        revision=getattr(args, "revision", None), use_fast=False,
    )
    noise_scheduler_1step = make_1step_sched(accelerator.device)
    ...
    vae_a2b, vae_lora_target_modules = initialize_vae(
        args.lora_rank_vae, device=accelerator.device, return_lora_module_names=True,
    )
    ...
    params_gen = CycleGANTurbo.get_trainable_params(unet, vae_a2b, vae_b2a)

    vae_enc = VAEEncode(vae_a2b, vae_b2a=vae_b2a)
    vae_dec = VAEDecode(vae_a2b, vae_b2a=vae_b2a)
    ...
    dataset_train = UnpairedTurboDataset(dataset_folder=args.dataset_folder, image_prep=args.train_img_prep, split="train", tokenizer=tokenizer)
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                """
                # A -> fake B -> rec A
                cyc_fake_b = CycleGANTurbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                cyc_rec_a = CycleGANTurbo.forward_with_networks(cyc_fake_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                ...
                # B -> fake A -> rec B
                cyc_fake_a = CycleGANTurbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                cyc_rec_b = CycleGANTurbo.forward_with_networks(cyc_fake_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                ...
                """
                fake_a = CycleGANTurbo.forward_with_networks(img_b, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                fake_b = CycleGANTurbo.forward_with_networks(img_a, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                ...
                Identity Objective
                """
                idt_a = CycleGANTurbo.forward_with_networks(img_b, "a2b", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_a2b_emb)
                loss_idt_a = crit_idt(idt_a, img_b) * args.lambda_idt
                loss_idt_a += net_lpips(idt_a, img_b).mean() * args.lambda_idt_lpips
                idt_b = CycleGANTurbo.forward_with_networks(img_a, "b2a", vae_enc, unet, vae_dec, noise_scheduler_1step, timesteps, fixed_b2a_emb)
                    ...
                    if global_step % args.validation_steps == 1:
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_src_test)):
                            ...
                            with torch.no_grad():
                                ...
                                eval_fake_b = CycleGANTurbo.forward_with_networks(img_a, "a2b", eval_vae_enc, eval_unet,
                        ...
                        for idx, input_img_path in enumerate(tqdm(l_images_tgt_test)):
                            ...
                            with torch.no_grad():
                                ...
                                eval_fake_a = CycleGANTurbo.forward_with_networks(img_b, "b2a", eval_vae_enc, eval_unet,
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Train CycleGAN-Turbo")
    ...
    parser.add_argument("--tracker_project_name", required=True)
...
class CycleGANTurboTrainer:
    """Training harness wrapping the upstream CycleGAN-Turbo loop."""

    def __init__(self, config: CycleGANTurboConfig) -> None:
        self.config = config

    def train(self) -> None:
        main(argparse.Namespace(**asdict(self.config)))


if __name__ == "__main__":
    cli = _parse_args()
    base = CycleGANTurboConfig()
    for k, v in vars(cli).items():
        if hasattr(base, k):
            setattr(base, k, v)
    CycleGANTurboTrainer(base).train()
