...
"""DiffusionRouter inference pipeline for universal multi-domain translation."""
...
from src.models.diffusionrouter import DiffusionRouterConfig, compose_route, parse_class
...
def _ensure_diffusionrouter_path(diffusionrouter_src_path: Optional[str | Path]) -> Path:
    ...
    candidates = [
        ...
        root / "_upstream_diffusionrouter",
        root / "projects" / "DiffusionRouter",
        Path.cwd() / "DiffusionRouter",
        Path.cwd() / "_upstream_diffusionrouter",
    ...
    raise FileNotFoundError(
        "DiffusionRouter source not found. Clone https://github.com/kvmduc/DiffusionRouter.git, "
        "set diffusionrouter_src_path, or place it at ./DiffusionRouter or ./_upstream_diffusionrouter."
    )
...
def load_diffusionrouter_pipeline(
    ...
    *,
    ...
    Requires a local checkout of `kvmduc/DiffusionRouter
    <https://github.com/kvmduc/DiffusionRouter>`_ for ``guided_diffusion``.

    Parameters
    ----------
    checkpoint_path : str or Path
        Path to a DiffusionRouter training or distillation checkpoint.
    diffusionrouter_src_path : str or Path, optional
        Path to the DiffusionRouter repo root. Auto-detected when omitted.
    config : DiffusionRouterConfig, optional
