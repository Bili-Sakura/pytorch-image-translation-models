...
import random
from src.models.cyclediff.taming.modules.losses.vqperceptual import *
...
class LatentDiffusion(DDPM):
    ...
    def reverse_q_sample(self, x):
        ...
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        time_steps = torch.cat((torch.tensor([time_steps[-1] - eps], device=device), time_steps[:-1]
                                ), dim=0)
        
        cur_time = torch.zeros((batch,), device=device) + eps
        C, noise = self.model(x_t, cur_time)
        x_t = self.q_sample(x_t, noise, cur_time, C)
        
        for id, time_step in enumerate(time_steps):
            s = torch.full((batch,), time_step, device=device)

            C, noise = self.model(x_t, cur_time)
            x0 = self.pred_x0_from_xt(x_t, noise, C, cur_time)
            C = -1 * x0

            cur_time = cur_time + s

            x_t = self.q_sample(x0, noise, cur_time, C)
    ...
    def reverse_q_sample_eps_list_aligned(self, x):
        """
        reference ddib
        """
        device = x.device
    ...
    def reverse_q_sample_eps_list_unaligned(self, x):
        """
        reference ddib
        """
        device = x.device
    ...
    def reverse_q_sample_c_list(self, x_start):
    ...
    def reverse_q_sample_c_list_concat(self, x_start):
        ...
        c_list.append(C)
        # c_list = torch.concat(c_list, dim=0)
        return c_list, x_t
    ...
    @torch.no_grad()
    def sample_origin_C(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z, C_list = self.sample_origin_C_fn((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise)
    ...
    @torch.no_grad()
    def sample_origin_C_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
    ...
    @torch.no_grad()
    def sample_after_C(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, C_list=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_after_C_fn((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
    ...
    @torch.no_grad()
    def sample_after_C_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, C_list=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
        ...
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        for i, (time_step, new_C) in enumerate(zip(time_steps, C_list)):
            s = torch.full((batch,), time_step, device=device)
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
    ...
    @torch.no_grad()
    def sample_from_eps(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, noise=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_eps((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, noise=noise)
    ...
    @torch.no_grad()
    def sample_fn_from_eps(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, noise=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
        ...
        for i, time_step in enumerate(time_steps):
            ...
            # img = self.pred_xtms_from_xt2(img, noise, C, cur_time, s)
            cur_time = cur_time - s
            img = self.q_sample(x0, noise, cur_time, C)
    ...
    def sample_from_c_list(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, c_list=None):
        ...
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_c_list((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, c_list=c_list)
    ...
    @torch.no_grad()
    def sample_fn_from_c_list(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, c_list=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
        ...
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        for i, (time_step, new_C) in enumerate(zip(time_steps, reversed(c_list))):
            s = torch.full((batch,), time_step, device=device)
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
    ...
    @torch.no_grad()
    def sample_from_c_list_eps_timestep(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, c_list=None, eps_timesteps=100):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_c_list_eps_timestep((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, c_list=c_list, eps_timesteps=eps_timesteps)
    ...
    @torch.no_grad()
    def sample_fn_from_c_list_eps_timestep(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, c_list=None, eps_timesteps=100):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
        ...
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        for i, (time_step, new_C) in enumerate(zip(time_steps[len(time_steps) - eps_timesteps:], reversed(c_list)[len(time_steps) - eps_timesteps:])):
            s = torch.full((batch,), time_step, device=device)
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
    ...
    @torch.no_grad()
    def sample_from_c_list_vq(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, c_list=None, code_book=None, codeformer=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_c_list_vq((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, c_list=c_list)

        #print(z.shape)
        vq_z = codeformer(z, vqcodebook=code_book)
        # vq_z = code_book.sample(z)
        if self.scale_by_std:
            vq_z = 1. / self.scale_factor * vq_z.detach()
        elif self.scale_by_softsign:
            vq_z = vq_z / (1 - vq_z.abs())
            vq_z = vq_z.detach()
        x_rec = self.first_stage_model.decode(vq_z)
        x_rec = unnormalize_to_zero_to_one(x_rec)
        x_rec = torch.clamp(x_rec, min=0., max=1.)
        if mask is not None:
            x_rec = mask * unnormalize_to_zero_to_one(cond) + (1 - mask) * x_rec
    ...
    @torch.no_grad()
    def sample_fn_from_c_list_vq(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, c_list=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
    ...
    @torch.no_grad()
    def sample_from_c_list_2(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, c_list=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_c_list_2((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, c_list=c_list)
    ...
    @torch.no_grad()
    def sample_fn_from_c_list_2(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, c_list=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
    ...
    @torch.no_grad()
    def sample_from_eps_list(self, batch_size=16, up_scale=1, cond=None, mask=None, denoise=True, eps_list=None):
        image_size, channels = self.image_size, self.channels
        if cond is not None:
            batch_size = cond.shape[0]
        down_ratio = self.first_stage_model.down_ratio
        z = self.sample_fn_from_eps_list((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                           up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise, eps_list=eps_list)
    ...
    @torch.no_grad()
    def sample_fn_from_eps_list(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False, eps_list=None):
        batch, device, sampling_timesteps = shape[0], self.eps.device, self.sampling_timesteps
        step = 1. / self.sampling_timesteps
        time_steps = torch.tensor([step], device=device).repeat(self.sampling_timesteps)
        if denoise:
        ...
        # K = -1 * torch.ones_like(img)
        cur_time = torch.ones((batch,), device=device)
        for i, (time_step, new_noise) in enumerate(zip(time_steps, reversed(eps_list))):
            s = torch.full((batch,), time_step, device=device)
            if i == time_steps.shape[0] - 1:
                s = cur_time
            if cond is not None:
