...
class TestMDTCommunity:
    """Tests for the MDT / LDDBM community pipeline."""

    def test_mdt_imports(self):
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput, load_mdt_community_pipeline

        assert MDTPipeline is not None
        assert MDTPipelineOutput is not None
        assert callable(load_mdt_community_pipeline)

    def test_load_mdt_without_lddbm_raises_helpful_error(self):
        """When lddbm is not installed, load_mdt_community_pipeline raises ImportError with MDT repo URL."""
        from examples.community.mdt import load_mdt_community_pipeline
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE

        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; skipping ImportError test")
        with pytest.raises(ImportError) as exc_info:
            load_mdt_community_pipeline("/tmp/checkpoints", device="cpu")
        assert "lddbm" in str(exc_info.value).lower()
        assert "Multimodal-Distribution-Translation-MDT" in str(exc_info.value)


class _TestMDTDuplicateToRemove:
    """Tests for the MDT / LDDBM community pipeline."""
    ...
    def test_import(self):
        ...
        assert MDTPipeline is not None
        assert MDTPipelineOutput is not None
        assert load_mdt_community_pipeline is not None

    def test_load_requires_lddbm_or_raises(self):
        from examples.community.mdt import load_mdt_community_pipeline

        try:
            load_mdt_community_pipeline("/tmp/nonexistent", device="cpu")
        except ImportError as e:
            assert "Multimodal-Distribution-Translation-MDT" in str(e) or "lddbm" in str(e).lower()
        except (FileNotFoundError, OSError, RuntimeError):
            # lddbm is installed; checkpoint loading failed (expected without real checkpoints)
            pass


class TestMDTCommunityPipeline:
    """Tests for the MDT / LDDBM community pipeline."""

    def test_import_mdt_pipeline(self):
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput, load_mdt_community_pipeline
    ...
    def test_load_mdt_requires_lddbm(self):
        from examples.community.mdt import load_mdt_community_pipeline
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE

        if not _LDDBM_AVAILABLE:
            with pytest.raises(ImportError) as exc_info:
                load_mdt_community_pipeline("/tmp/no-checkpoints")
            assert "lddbm" in str(exc_info.value).lower()
            assert "Multimodal-Distribution-Translation-MDT" in str(exc_info.value)
...
class TestMDTCommunity:
    ...
    def test_load_requires_lddbm_or_raises(self):
        ...
        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; skipping load-failure test")
        with pytest.raises(ImportError) as exc_info:
            load_mdt_community_pipeline("/tmp/does-not-exist")
        assert "lddbm" in str(exc_info.value).lower()
        assert "Multimodal-Distribution-Translation-MDT" in str(exc_info.value)


class TestMDTPipeline:
    """Tests for the MDT / LDDBM community pipeline."""
    ...
    def test_load_mdt_requires_lddbm(self):
        ...
        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; would need real checkpoints to test load")
            return
        with pytest.raises(ImportError, match="lddbm|MDT"):
            load_mdt_community_pipeline("/nonexistent/path", device="cpu")
...
class TestMDTCommunityPipeline:
    """Tests for the MDT community pipeline (requires lddbm package)."""

    def test_mdt_imports(self):
        """MDTPipeline and related symbols can be imported without lddbm."""
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput, load_mdt_community_pipeline
    ...
    def test_load_mdt_without_lddbm_raises_helpful_error(self):
        """Without lddbm installed, load_mdt_community_pipeline raises ImportError with MDT repo URL."""
        import examples.community.mdt.pipeline as mdt_pipeline

        if mdt_pipeline._LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; cannot test error message")
            return

        with pytest.raises(ImportError) as exc_info:
            mdt_pipeline.load_mdt_community_pipeline("/tmp/checkpoints", device="cpu")

        assert "Multimodal-Distribution-Translation-MDT" in str(exc_info.value)
        assert "git clone" in str(exc_info.value) or "pip install" in str(exc_info.value)


class TestMDTPipeline:
    """Tests for the MDT / LDDBM community pipeline."""

    def test_import_mdt_symbols(self):
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput, load_mdt_community_pipeline
    ...
    def test_load_mdt_raises_without_lddbm(self):
        """When lddbm is not installed, load_mdt_community_pipeline raises a clear ImportError."""
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE

        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; skip ImportError test")
            return
...
class TestMDT:
    ...
    def test_load_without_lddbm_raises_helpful_error(self):
        """When lddbm is not installed, load_mdt_community_pipeline raises ImportError with install instructions."""
        from examples.community.mdt import load_mdt_community_pipeline
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE

        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; cannot test ImportError path")
        with pytest.raises(ImportError) as exc_info:
            load_mdt_community_pipeline("/tmp/checkpoints", device="cpu")
        assert "lddbm" in str(exc_info.value).lower() or "MDT" in str(exc_info.value)
...
class TestMDT:
    ...
    def test_import_mdt_pipeline_and_output(self):
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput

        assert MDTPipeline is not None
        assert MDTPipelineOutput is not None

    def test_import_load_mdt_community_pipeline(self):
        from examples.community.mdt import load_mdt_community_pipeline

        assert callable(load_mdt_community_pipeline)

    def test_load_mdt_community_pipeline_requires_lddbm(self):
        """When lddbm is not installed, load_mdt_community_pipeline raises ImportError with helpful message."""
        from examples.community.mdt import load_mdt_community_pipeline
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE
        ...
        with pytest.raises(ImportError) as exc_info:
            load_mdt_community_pipeline("/tmp/nonexistent", device="cpu")
        assert "lddbm" in str(exc_info.value).lower()
        assert "github.com/boschresearch" in str(exc_info.value)
...
class TestMDTCommunity:
    """Tests for the MDT community pipeline (requires lddbm when loading)."""

    def test_mdt_imports(self):
        """MDTPipeline and MDTPipelineOutput can be imported without lddbm."""
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput, load_mdt_community_pipeline
    ...
    def test_load_mdt_raises_without_lddbm(self):
        """load_mdt_community_pipeline raises helpful ImportError when lddbm is not installed."""
        from examples.community.mdt import load_mdt_community_pipeline
        from examples.community.mdt.pipeline import _LDDBM_AVAILABLE
        ...
        if _LDDBM_AVAILABLE:
            pytest.skip("lddbm is installed; cannot test ImportError path")
        with pytest.raises(ImportError) as exc_info:
            load_mdt_community_pipeline("/tmp/checkpoints")
        assert "lddbm" in str(exc_info.value).lower()
        assert "Multimodal-Distribution-Translation-MDT" in str(exc_info.value)
...
class TestMDT:
    ...
    def test_import_without_lddbm(self):
        """load_mdt_community_pipeline raises ImportError when lddbm is not installed."""
        from examples.community.mdt import load_mdt_community_pipeline

        try:
            load_mdt_community_pipeline("/nonexistent", device="cpu")
        except ImportError as e:
            assert "lddbm" in str(e).lower() or "MDT" in str(e)
        except (FileNotFoundError, OSError):
            # lddbm may be installed; loading fails due to missing checkpoints
            pass

    def test_mdt_pipeline_output_type(self):
        """MDTPipelineOutput and MDTPipeline are importable."""
        from examples.community.mdt import MDTPipeline, MDTPipelineOutput
...
class TestMDTCommunityPipeline:
    ...
    def test_import_without_lddbm(self):
        """When lddbm is not installed, load_mdt_community_pipeline raises ImportError."""
        try:
            from examples.community.mdt.pipeline import _LDDBM_AVAILABLE
            if _LDDBM_AVAILABLE:
                pytest.skip("lddbm is installed; skip import-error test")
        except ImportError:
            pytest.skip("Cannot import MDT pipeline")
